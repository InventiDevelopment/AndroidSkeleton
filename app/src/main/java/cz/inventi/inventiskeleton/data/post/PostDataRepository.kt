 ...
 */
class PostDataRepository @Inject constructor(val remoteStore: RemotePlaceholderService, val localStore: PostDao) : PostRepository {
    ...
    override fun post(postId: Int): Observable<Post> {
        val remotePost: Flowable<Post> = remoteStore.post(postId).toFlowable() // TODO update individual post in db
        val localPost: Flowable<Post> = localStore.findById(postId)
        return Flowable.merge(remotePost, localPost).toObservable() // TODO error handling an fix multiple return
    }
    ...
    override fun postList(): Observable<List<Post>> {
        ...
        val remotePostList = remoteStore.postList()
                .doOnNext { localStore.insertAll(test, test) }
//                .map { listOf(PostEntity(1, 1, "From remote title", "From remote body")) + it } // for testing, add new remote element as first

        val localPostList = localStore.all.toObservable()
